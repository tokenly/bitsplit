...
class WebhookCaller {
    ...
    public function sendWebhook($event_type, User $user, $payload_vars=[]) {
        // get the webhook (if any)
        $webhook_url = UserMeta::getMeta($user['id'], 'webhook_url');
        if (!$webhook_url) {
            EventLog::debug('webhook.noWebhookForUser', ['event' => $event_type, 'userId' => $user['id']]);
            return;
        }
